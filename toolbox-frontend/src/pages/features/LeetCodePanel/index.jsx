...
function Row({ row: { question, subQuestions }, onUpdate, onDelete, onEdit }) {
    ...
    function tagsColumn(id, tags, setTags) {
        return (
            <Autocomplete
                sx={{ maxWidth: 300 }}
                multiple
    ...
    function ratingColumn(id, rating, setRating) {
        return (
            <Rating
    ...
    function statusColumn(id, status, setStatus) {
    ...
    function _row(row, isSub = false) {
        const [rating, setRating] = React.useState(row?.proficiencyRating);
        const [tags, setTags] = React.useState(row?.questionTags?.split(','));
        const [status, setStatus] = React.useState(row.status);
        return <TableRow sx={{ '& > *': { borderBottom: 'unset' }, display: isSub ? open ? '' : 'none' : '' }}>
            <TableCell>
                <IconButton
            ...
            <TableCell align="left"><Typography variant='body1'>{ratingColumn(row.id, rating, setRating)}</Typography></TableCell>
            <TableCell align="left">{tagsColumn(row.id, tags, setTags)}</TableCell>
            <TableCell align="left">{statusColumn(row.id, status, setStatus)}</TableCell>
    ...
    return (
        <React.Fragment>
            {_row(question)}
            {
                subQuestions.map(q => _row(q, true))
            }
        </React.Fragment>
    );
}
