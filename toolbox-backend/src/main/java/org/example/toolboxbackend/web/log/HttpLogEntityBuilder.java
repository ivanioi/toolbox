...
public class HttpLogEntityBuilder {
     ...
     * @return
     */
    public static HttpLogEntity build(ContentCachingRequestWrapper requestWrapper, ContentCachingResponseWrapper responseWrapper, StopWatch stopWatch) throws JsonProcessingException {
        ...
        if (requestWrapper.getMethod().equals(RequestMethod.GET.name())) {
            httpLogEntity.setRequestParams(new ObjectMapper().writeValueAsString(requestWrapper.getParameterMap()));
        } else {
        ...
        if (StringUtils.equalsAny("application/json", "application/json;charset=UTF-8", responseContentType)) {
